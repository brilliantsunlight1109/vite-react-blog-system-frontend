  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response2 = await axios.get("/api/stylist");
        const response3 = await axios.get("api/coupon");
        const contributor_user = response2.data.map((item) => item.name);
        const coupon_select = response3.data.map((item) => item.coupon_name);
